...
Basket Size: exclude transaksi amount=0 dan item group Reseller
...
def is_store2_item(name):
    """Store 2 = item_name mengandung '2nd store'"""
    return "2nd store" in str(name).lower()

def is_reseller_order(items):
    """Reseller = item_group == 'Reseller' (case-insensitive)"""
    return any(
        str(i.get("item_group") or i.get("category") or "").strip().lower() == "reseller"
        for i in items
    )
...
def process_orders(orders):
    ...
    for o in orders:
        tgl = str(
            o.get("order_date") or o.get("order_date_time") or
            o.get("created_at") or ""
        )[:10]
        ...
        items = (
            o.get("items") or o.get("order_items") or
            o.get("details") or o.get("order_details") or []
        )

        # Tentukan store dari item
        order_is_s2 = any(
            is_store2_item(i.get("item_name") or i.get("product_name") or "")
            for i in items
        )
        ...
        # Cek apakah order ini Reseller
        order_is_reseller = is_reseller_order(items)
        ...
        # ── Per store ──
        ds = daily_s2 if order_is_s2 else daily_s1
        hs = hourly_s2 if order_is_s2 else hourly_s1
        ...
        for item in items:
            ...
            # Tampilkan nama bersih tanpa "2nd Store"
    ...
    # Product mix donut (top 4 + Others)
    mix_sorted = sorted(produk_all.items(), key=lambda x: x[1]["revenue"], reverse=True)
    top4       = mix_sorted[:4]
    others_rev = sum(v["revenue"] for _, v in mix_sorted[4:])
    product_mix = [
        {"name": n, "revenue": round(v["revenue"], 0),
         "pct": round(v["revenue"] / tot_rev * 100, 1) if tot_rev else 0}
        for n, v in top4
    ]
    if others_rev > 0:
        product_mix.append({
            "name": "Others",
            "revenue": round(others_rev, 0),
            "pct": round(others_rev / tot_rev * 100, 1) if tot_rev else 0
        })

    gpm = lambda p, r: round(p / r * 100, 1) if r else 0

    # Basket size: gunakan basket_rev / basket_txn (exclude 0 dan Reseller)
    ...
    return {
        ...
        "basket_txn_count": basket_txn,  # jumlah order yang masuk basket size
        "gpm_total":       gpm(tot_prof, tot_rev),

        "store1": {
            "name":      "Jl. Yos Sudarso IV",
            "revenue":   round(s1_rev, 0),
            "profit":    round(s1_prof, 0),
            "transaksi": int(s1_txn),
            "gpm":       gpm(s1_prof, s1_rev)
        },
        "store2": {
            "name":      "Jl. Diponegoro",
            "revenue":   round(s2_rev, 0),
            "profit":    round(s2_prof, 0),
            "transaksi": int(s2_txn),
            "gpm":       gpm(s2_prof, s2_rev)
        },
...
def _empty():
    ...
    return {
        ...
        "basket_size": 0, "basket_txn_count": 0, "gpm_total": 0,
...
def make_insights(curr, growth):
    ...
    if rev_g is not None:
        if rev_g >= 0:
            ins.append({
                ...
                "title": f"Revenue naik {rev_g}% periode ini",
    ...
    # Store comparison
    s1g = growth.get("store1_revenue")
    s2g = growth.get("store2_revenue")
    if s1g is not None and s2g is not None:
        if s2g > s1g and s2g > 0:
            ins.append({
                "type": "positive", "icon": "flash",
                "title": "Store 2 (Diponegoro) tumbuh lebih cepat",
                "message": f"Jl. Diponegoro tumbuh {s2g}% vs Store 1 yang {s1g}%."
            })
        elif s1g > s2g and s1g > 0:
            ins.append({
                "type": "positive", "icon": "flash",
    ...
    # GPM compression
    gpm_s1 = growth.get("gpm_s1_pp")
    gpm_s2 = growth.get("gpm_s2_pp")
    for store_name, diff in [("Store 1", gpm_s1), ("Store 2", gpm_s2)]:
        if diff is not None and diff < -0.5:
            ins.append({
                "type": "alert", "icon": "warning",
                "title": f"Margin compression — {store_name}",
                "message": f"GPM turun {abs(diff)}pp. Perlu review COGS dan harga beli."
            })
            break
...
def refresh_all():
    ...
    try:
        ...
        print(f"[OK] Weekly: Rp {wr:,.0f} | Monthly: Rp {mr:,.0f} | Basket: Rp {wb:,.0f}")
...
def api_dashboard():
    ...
    if not data:
        ...
        return jsonify({
            "status":  "loading",
            "message": "Mengambil data Olsera, coba lagi dalam 30 detik"
        }), 503
    return jsonify(data)
...
def api_status():
    return jsonify({
        ...
        "basket_size_weekly": _cache['weekly'].get('basket_size') if _cache['weekly'] else None,
    })
...
def index():
    ...
    m = _cache['monthly']
    return jsonify({
        "service": "Keyos Dashboard — Olsera Middleware",
        "status":  "online",
        "endpoints": {
            "/api/dashboard?period=weekly":  "Data minggu ini vs minggu lalu",
            "/api/dashboard?period=monthly": "Data bulan ini vs bulan lalu",
            "/api/refresh":                  "Trigger refresh manual",
            "/api/status":                   "Status server & cache",
        },
